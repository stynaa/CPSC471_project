...
if($username != -1) {
    if ($stmt->execute()) {
        ...
        echo "<table>";
        echo '<th>First Name</th><th>Last Name</th><th>Date of Birth</th><th>Student ID</th><th>View Classes</th><th>View Sessions</th><th>View Schedule</th>';
        while($x = $result->fetch_assoc()) {
            echo '<tr>' . '<td>'.$x["first_name"].'</td><td>'.$x["last_name"].
                ...
                '</td><td><button onclick="viewSessions(' .$x["student_id"] . ')">View Sessions</button>'.
                '</td><td><button onclick="viewScheduleAsStudent(' .$x["student_id"] . ')">View Schedule</button></td>'.
                '</tr>';
        }
        echo "</table>";
      } else {
